...
| 🖼️ **手写便签图片** | 纸条自动渲染成「横线活页纸 + 红心 + 手写签名」的便签 PNG，**手写字体可选** |
...
| `font_family` 便签字体 | 枚举 | `华文行楷` | **手写体下拉选择**，见下文字体说明 |
...
> 系统缺某个字体时自动按手写感优先级回退（先静态路径查找，再扫描系统字体目录按文件名关键词匹配），找不到就用默认字体。
...
- **正文**：手写字（默认华文行楷，可换字体；自动换行，最多 4 行 + 省略号）
...
| 字体 | SkiaSharp 固定楷体 | **手写字体 enum 可选**（默认华文行楷） |
| 渲染 | SkiaSharp | Pillow |
