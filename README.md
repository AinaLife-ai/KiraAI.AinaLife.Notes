...
| 功能 | 说明 |
|---|---|
| 📝 **留纸条** | 让 AI 随时留一张纸条（温柔话、提醒、心情、待办…） |
| 📋 **看纸条** | 列出所有纸条（带时间），按序号展示 |
| 🗑️ **删纸条** | 按序号删除指定纸条 |
| 🧹 **清空纸条** | 一键清空所有纸条 |
| ⏰ **自动温柔纸条** | 按定时规则由 **AI 自主生成**一句暖心话并保存，不打断当前对话 |
| 🖼️ **手写便签图片** | 纸条自动渲染成「横线活页纸 + 红心 + 手写签名」的便签 PNG，发送到群聊 / 私聊 |
...
> 💡 目录名必须叫 `ainalife_notes`，否则框架会按目录名当 plugin_id 解析，配置会错位。
...
| `auto_prompt` 自动纸条提示词 | 多行文本 | `""` | 留空使用内置默认提示词（直接输出一句 50 字内暖心话） |
| `gen_model` 生成纸条的模型 | 模型选择 | `""` | 留空 = 跟随 KiraAI 默认模型 |
| `signature` 便条签名 | 文本 | `爱奈丽` | 便签图片右下角的手写签名 |
| `send_target_type` 纸条图片发送类型 | 枚举 | `none` | `none`=仅存档不发送；`group`=发送到群聊；`private`=发送到私聊 |
| `send_target_id` 纸条图片发送目标 | 文本 | `0` | 群号或 QQ 号，与发送类型配合；`0` 表示不发送 |
| `send_adapter` 发送适配器实例名 | 文本 | `qq` | 会话标识第一段（如 `qq`），必须与你的适配器实例名一致 |
...
| 工具 | 说明 |
|---|---|
| `note_add(content)` | 留一张纸条（配置了发送目标时同时渲染成便签图片发出） |
| `note_list()` | 查看所有纸条 |
| `note_delete(index)` | 删除第 N 张纸条（从 1 开始） |
| `note_clear()` | 清空所有纸条 |

> 你可以直接在对话里说「给我留个纸条：记得喝水」之类的话，AI 会自己调用 `note_add`。

## 🖼️ 便签图片长什么样

- **背景**：米色横线活页纸（浅蓝横线 + 左侧打孔）
- **标题**：自动纸条写「温柔小纸条」，手动纸条写「小纸条」，标题右侧有小爱心
- **正文**：深灰色手写字（自动换行，最多 4 行 + 省略号）
- **右下角**：红色大爱心 + 蓝色手写签名（默认「爱奈丽」，可配置）

图片渲染使用 **Pillow**（自动安装），字体优先尝试 `楷体(KaiTi) → 黑体 → 微软雅黑`，找不到就用默认字体。
...
手写 JSON 原子写入（临时文件 + 替换），断电/崩溃也不会损坏。

## 🧠 工作原理

- **定时器**：插件启动后后台每 30 秒检查一次；到点后**先掷出下一次时间并落盘**（防止生成过程中重复触发），再调用 LLM 生成纸条内容，全程**不经过对话流水线**，不打断正在进行的对话
- **LLM 生成**：直接调用 `ctx.get_default_llm_client()`（或你指定的 `gen_model`），System 提示词为「你是一个温柔的数字生命」，输出一句 ≤50 字的暖心话
- **发送**：通过 `message_processor.send_message_chain` 发送图片，跨平台（QQ / Telegram / Discord 等均可用）
...
# 手动跑一个便签渲染测试（需在 KiraAI 环境内，Pillow 已装）
...
[GNU Affero General Public License v3.0](LICENSE) — 网络服务使用本插件也需开放源码（AGPL 特性）。
